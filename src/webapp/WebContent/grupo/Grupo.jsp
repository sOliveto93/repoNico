...
var idFilaAnterior;
	...
	function openDialogPartida(operacion,cargarDescripcion){
		...
		if( operacion =="modificarPartidaGrupoJson" && cargarDescripcion == "true"){
			$("#dialogFormPartida").dialog({title: "Modificar Partida"});
			var idPartida = $("#partida").val();
			$("#descripcion").val($("#partida option:selected").text());
			$("#idPartida").val(idPartida);

			
		}else if(operacion =="altaPartidaGrupoJson" && cargarDescripcion == "false"){
			$("#dialogFormPartida").dialog({title: "Nueva Partida"});
			limpiarForm('partidaAltaModifica');
			$("#descripcion").val("");
			$("#idPartida").val("");
		}
	...
	function calcularMora(){
		if($("#cobraMora").attr('checked')){ 
			var vencimientoMonto = ((parseInt($("#conceptoUnoMonto").val())	+ parseInt($("#conceptoDosMonto").val())) * 6.41 /100);
			$("#vencimientoMonto").val(vencimientoMonto.toFixed(2));
				/*$("#vencimientoMonto").val((parseInt($("#conceptoUnoMonto").val())+parseInt($("#conceptoDosMonto").val()))* 6.41 /100);*/
		}else{
...
<div id="dialogFormPartida" title="Alta Partida">
	<form name="partidaAltaModifica" id="partidaAltaModifica">
		Descripcion: <input type="text" name="descripcion" id="descripcion"  value=""></input>
		...
		<c:forEach items="${grupos}"  var="grupo">
			...
			<tr id="${grupo.id}">
				...
				<c:if test="${usuario.tipo == ADM}">
					...
					<td><a href="javascript:eliminar('${grupo.id}')"><img src="images/delete.png" width="24px" title="Eliminar Grupo" /></a></td>
				</c:if>
				<c:if test="${usuario.tipo == AUDITORIA}">
					<td><a href="javascript:alert('operacion deshabilitada');"><img src="images/pencilGris.png" width="24px" title="Modificar Grupo"/></a></td>
					<td><a href="javascript:alert('operacion deshabilitada');"><img src="images/checkGris.png" width="24px" title="Generar Chequeras"/></a></td>
					<td><a href="javascript:alert('operacion deshabilitada');"><img src="images/signoPesoEditarGris.png" width="24px" title="Modificar Cuotas"/></a></td>
					<td><a href="javascript:alert('operacion deshabilitada');"><img src="images/signoPesoViewGris.png" width="24x" title="Vista de Cuotas"/></a></td>
					<td><a href="javascript:alert('operacion deshabilitada');"><img src="images/deleteGris.png" width="24px" title="Eliminar Grupo" /></a></td>
				</c:if>
			</tr>
			</c:if>
		</c:forEach>
	...
	<form action="GrupoServlet" method="post" name="edicion" id="edicion">
		<table>
			<tr>
				...
				<td colspan="3">
					<input name="codigo" id="codigo" size="7" maxlength="7" onblur="javascript:buscarGrupoPorCodigo(this.value);">
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="3">
					...
					<select name="carreraGrupo" id="carreraGrupo">
		...
		<table width="721" border="0">
			<tr>
				...
				<td colspan="2">
					Partida <select name="partida" id="partida">
						<c:forEach items="${partidas}"  var="partida">
							<option value="${partida.id}">${partida.descripcion}</option>
						</c:forEach>
					</select>
					<a href="javascript:openDialogPartida('altaPartidaGrupoJson','false');"><img src="images/add.png" width="15px" title="Agregar Partida"/></a>
			...
			<tr>
				...
				<td colspan="2">
				<select name="tipoMora" id="tipoMora">
			...
			<tr>
				...
				<td>
					<input name="diasSegundoVencimiento" id="diasSegundoVencimiento" value="" size="10" maxlength="10">
				</td>
				<td>
					Cantidad de Cuotas
				</td>
			...
			<tr>
				...
				<td>
					<input type="checkbox" id="tramite_ingreso" name="tramite_ingreso" >
				</td>
				<br>
				<td>
					tramite grado
				...
				<td>
					<input type="checkbox" id="tramite_grado" name="tramite_grado" ">
				</td>
				<br>
					<td>
					tramite posgrado
				...
				<td>
					<input type="checkbox" id=tramite_posgrado name="tramite_posgrado" >
				</td>
			</tr>
			 -->
		</table>
