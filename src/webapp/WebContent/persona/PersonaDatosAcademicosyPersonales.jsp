...
<form action="PersonaServlet" method="post" name="edicion" id="edicion" >
	<fieldset><legend>Datos Personales</legend>
		<table id='datosPersonales'>
			...
			<tr>
				...
				<td>Telefono:<input name='telefono' id='telefono' value='${persona.telefono}'></td>
			</tr>
			<tr>
	...
	<fieldset><legend>Datos Academicos</legend>
		<table id='datosAcademicos'>
			<tr> 	
				<td colspan="1">
					...
					Fecha de Ingreso:
					<input name='fechaIngreso' id='fechaIngreso' value='${persona.fechaIngreso}'>
				</td>
							...
							<c:forEach items="${persona.inscripciones}" var="inscripcion">
								<li>
									...
									<c:choose>
									 ...
									 <a href="javascript:modificarInscripcion('borrarInscripcion','${persona.id}','${inscripcion.carrera.id}','${inscripcion.id}','${operacion}');">
									 <img src="images/delete.png" width="15px" title="Eliminar Carrera" />
									 </a> 
								</li>
							</c:forEach>
						</ul>
		...
		<fieldset><legend>Becas</legend>
			<table>
				<tr>
					<td>
						<button name="beca" id="beca">Beca</button>
					</td>
					<td>
						<select name='becasPersona' id="becasPersona"  size='5' ondblclick="javaScript:buscaDescuentoPorInscripcion(this.value)">
							<option value='0'>Sin Beca</option>
							<c:forEach items="${personasBecas}" var="personasBeca">
								<c:if test="${not empty  personasBeca}"> 
									<option value='${personasBeca.id}' selected="selected">${personasBeca.inscripcion.carrera.nombreCarrera} - ${personasBeca.beca.descripcion} - ${personasBeca.descuentoBeca}%  </option>
								</c:if>
							</c:forEach>
						</select>
					</td>
...
<div id="dialogFormMail" title="Mail">
	<form name="edicionMail" id="edicionMail">
		Mail: <input type="text" name="mailEdit" id="mailEdit"  value='${persona.mail}'>
