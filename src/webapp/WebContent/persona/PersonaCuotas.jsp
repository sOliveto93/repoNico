				...
				<tr id="${pago.id}">
					...
					<td align="center">
						...
						<c:if test="${(pago.estado.activado) && (empty pago.fechaPgo)}">
							<a href="javascript:openDialogCuota(${pago.monto1},${pago.monto2},${pago.vtoMonto},'${pago.fecha}','${pago.tipo}',${pago.id},${pago.exceptuarMora});"><img src="images/pencil.png" width="15px" title="Modificar Cuota"/></a>
						</c:if>
					</td>
						<td align="center">
						<c:if test="${pago.estado.valor == 'Incierto' || (not empty pago.fechaPgo)}">
							<img src="images/deleteGris.png" width="15px" title="Cuota Paga" />				
						</c:if>
						<c:if test="${(pago.estado.activado) && (empty pago.fechaPgo)}">
							<a href="javascript:registrarDeleteSiNo('${pago.id}','${pago.persona.id}')" title="Eliminar Cuota"><img src="images/delete.png" width="15px" title="Eliminar Cuota" /></a>					
						</c:if>
					</td>
					<td>
						<c:if test="${pago.estado.valor == 'Incierto' || (not empty pago.fechaPgo)}">
							<a href="javascript:anulaPagoPersona('${pago.id}','${persona.id}');" title="Anular Pago Cuota"><img src="images/pesosDelete.png" width="15px" title="Anular Pago Cuota" /></a>
							
							<!-- <input type="checkbox" name="anularPagoCuota" id="anularPagoCuota" value="${pago.id}"> -->						
						</c:if>
						<c:if test="${(pago.estado.activado) && (empty pago.fechaPgo)}">
							<img src="images/pesosDeleteGris.png" width="15px" title="Anular Pago Cuota" />
							<!-- <input type6="checkbox" name="anularPagoCuota" id="anularPagoCuota" value="${pago.id}" disabled="disabled"> -->					
						</c:if>
					</td>
				</tr>
			</c:if>
...
<table id="botones">
	<tr>
		...
		<td>
			<button name="cupones" id="cupones" >Generar y Facturar Cupones</button>
		</td>
	</tr>
</table>
